...
/*   Updated: 2019/10/15 20:17:22 by hkuhic           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_sphere      *exec_sphere_1(void)
{
  ...
  nbrs_sph = NUMB_SPHERE_IN_SCENE1;
  ...
  spheres[1] = sphere_new(vec_new(1, 0, 3), 1, 500, vec_new(0,0,255));
  spheres[2] = sphere_new(vec_new(-1, 0, 3), 1, 10, vec_new(0,255,0));
...
t_cone      *exec_cone_1(void)
{
  ...
  cone[0] = cone_new(vec_new(0, 0, 6), v, 250, vec_new(255, 100, 100), 0.2);
...
t_cylin      *exec_cylinder_1(void)
{
  ...
  free (ivory.type_mat);
  v = vec_new(4, 2, 2);
  v = vec_new(v.x - 0, v.y - 0, v.z - 4);
  v = vec_new(v.x / vec_length(v), v.y / vec_length(v), v.z / vec_length(v));
  cylin[0] = cylin_new(vec_new(1, 0, 2), v, 1, 250, vec_new(100,100,255));
  //cone[1] = cone_new(vec_new(2, 0, 4), vec_new(2, 0, 5), 500, vec_new(0,0,255), 0.2);
  // cone[2] = cone_new(vec_new(-2, 0, 4), 1, 10, vec_new(0,255,0));
  // cone[3] = cone_new(vec_new(0, -5001, 0), 5000, 1000, vec_new(255,255,0));
  return (cylin);
...
t_all_obj     exec_obj_1(void)
{
  ...
  // objs.new_cylindr = exec_cylindr_1();
  return (objs);
}
...
*/
void    init_obj_1(t_obj    ***all_obj, int *num)
{
  t_all_obj   objs;
  int         i;

  objs = exec_obj_1();
  *num = NUMB_OBJ_IN_SCENE1;
  if (!((*all_obj) = (t_obj **)malloc(sizeof(t_obj*) * *num)))
    ft_print_error_exit(&ft_putendl, "Error, no_obj");
  i = -1;
  while (++i < NUMB_OBJ_IN_SCENE1)
  {
    if (!((*all_obj)[i] = (t_obj *)malloc(sizeof(t_obj))))
      ft_print_error_exit(&ft_putendl, "Error, no_obj_elem");
  }
  i = 0;
  fill_objs_sphere(all_obj, objs.new_sphere, &i, NUMB_SPHERE_IN_SCENE1);
  fill_objs_cone(all_obj, objs.new_cone, &i, NUMB_CONE_IN_SCENE1);
  //fill_objs_plane(all_obj, objs.new_plane, &i, NUMB_PLANE_IN_SCENE1);
  //fill_objs_cylin(all_obj, objs.new_cylindr, &i, NUMB_CYLINDR_IN_SCENE1);
...
void    scene1(t_sdl **sdl)
{
  ...
  init_obj_1(&((*sdl)->obj), &((*sdl)->num_obj));
