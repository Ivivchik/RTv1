...
/*   Updated: 2019/10/16 17:31:25 by hkuhic           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "rtv1.h"

double    *cast_ray_for_obj_shadow(t_vec3 a, t_vec3 b, t_sdl *sdl, int i)
{
  double      *t;
  double        tp;

  if (ft_strcmp(sdl->obj[i]->c_type, STR_SPHERE) == 0)
    t = intersec_sphere(a, b, get_sph(sdl->obj[i]->type), sdl);
  else if (ft_strcmp(sdl->obj[i]->c_type, STR_CONE) == 0)
    t = intersec_conus(a, b, get_con(sdl->obj[i]->type), sdl);
  else if (ft_strcmp(sdl->obj[i]->c_type, STR_PLANE) == 0)
      tp = intersec_plane(a, b, get_pla(sdl->obj[i]->type), sdl);
  else if (ft_strcmp(sdl->obj[i]->c_type, STR_CYLINDR) == 0)
      t = intersec_cylinder(a, b, get_cyl(sdl->obj[i]->type), sdl);
  return (t);
}
...
double		computer_lighting(t_vec3 p, t_vec3 n, t_sdl *rt)//, int i)//, t_vec3 v, int s)
{
	t_light l[5];
	t_vec3 li;
	t_vec3 r;
...
t_vec3      calc_color(double close_t, t_vec3 b, t_sdl *sdl, int i)
{
 ...
 // n = vec_scale(vec_new(p.x - sdl->f_center.x, p.y - sdl->f_center.y, p.z - sdl->f_center.z), -1);
  ...
  //col_out = sdl->f_color;
  col_out = vec_scale(sdl->f_color, computer_lighting(p, n, sdl));//, i));//, vec_scale(b, -1), sdl->f_blesk));
  return (col_out);
}
